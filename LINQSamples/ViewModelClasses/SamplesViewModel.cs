...
namespace LINQSamples
{
  public class SamplesViewModel
  {
    ...
    {
      List<Product> list = new List<Product>(); //list of product
      foreach(Product item in Products)
            {
                list.Add(item);//puting in to another list
            }
    ...
    public void OrderByTwoFields()
    {
      if (UseQuerySyntax) {
                // Query Syntax
                Products = (from prod in Products
                            orderby prod.Name descending, prod.Name
                            select prod).ToList();
      ...
      public void WhereExpression()
        {
            ...
            {
                Products = (from prod in Products
                            where prod.Name.StartsWith(search) select prod).ToList();
            }
            else
            {
